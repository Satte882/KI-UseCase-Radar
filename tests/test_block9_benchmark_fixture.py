...
def test_block9_benchmark_freezes_interactive_run_order_and_count():
    fixture = _load_fixture()
    operator = fixture["operator_contract"]
...
def test_block9_benchmark_has_fixed_primary_end_state_and_gate_boundary():
    fixture = _load_fixture()
    end_state = fixture["shared_end_state"]
...
def test_block9_benchmark_keeps_path_specific_start_states_explicit():
    ...
    assert paths["accelerator"]["seed_fields_excluded_from_scoring"] == [
        "business_unit",
        "business_owner",
        "submitter",
        "status",
...
def test_block9_robustness_case_contains_all_required_challenges():
    ...
    assert quality["must_not_invent"] == [
        "provider",
        "model_name",
        "support_responsibility",
    ]
    assert quality["number_unit_pairs"] == [
        {"field": "metric_baseline", "value": "11", "unit": "Minuten"}
    ]
