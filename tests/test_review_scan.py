...
def test_review_scan_records_job(owner, business_unit):
    UseCase.objects.create(title="Überfällig", problem_statement="Problem", business_unit=business_unit, affected_process="Prozess", business_owner=owner, expected_benefit="Nutzen", next_review_date=timezone.localdate() - timezone.timedelta(days=1))
