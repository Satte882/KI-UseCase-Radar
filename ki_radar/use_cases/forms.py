...
class UseCaseForm(forms.ModelForm):
    class Meta:
        model = UseCase
        exclude = [
            "id", "short_id", "status", "submitter", "created_at", "updated_at", "actual_end_date",
            "ending_reason", "final_assessment", "lessons_learned", "data_and_access_handling",
            "replacement_solution", "is_archived",
            "privacy_review_required", "security_review_required", "legal_review_required",
        ]
        widgets = {
    ...
    def __init__(self, *args, current_user=None, **kwargs):
        ...
        for field in self.fields.values():
            field.widget.attrs.setdefault("class", "form-control" if not isinstance(field.widget, forms.CheckboxInput) else "form-check-input")
        self.fields["business_unit"].queryset = BusinessUnit.objects.filter(is_active=True)
        User = get_user_model()
        active_users = User.objects.filter(is_active=True, is_anonymized=False).order_by("last_name", "first_name", "username")
        ...
        if current_user and not is_coordinator(current_user):
            for name in ["business_owner", "coordinator", "privacy_review_completed", "security_review_completed", "legal_review_completed"]:
    ...
    def clean(self):
        ...
        if start and end and end < start:
            self.add_error("planned_pilot_end", "Das geplante Pilotende darf nicht vor dem Pilotbeginn liegen.")
        return cleaned
